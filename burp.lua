-- =========================================================
-- ANTI-SPY / GUI DETECTION REMOVER
-- Fungsi: Mendeteksi Logger, menghapus GUI, dan Kick/Teleport
-- =========================================================
...
-- 1. Hitung mundur 2 detik sebelum melakukan pemeriksaan
task.wait(2)

local function isSpyDetected()
    local detected = false

    -- METODE A: Mendeteksi apakah fungsi API Exploit di-hook secara langsung
    -- Mengecek dukungan fungsi 'isfunctionhooked' pada executor
    if type(isfunctionhooked) == "function" then
        pcall(function()
            if isfunctionhooked(loadstring) or isfunctionhooked(game.HttpGet) then
                detected = true
            end
        end)
    end

    -- METODE B: Mengecek C-Closure
    -- Logger biasanya menggunakan 'hookfunction' dengan fungsi Lua biasa (L-Closure).
    -- Fungsi bawaan Roblox/Executor adalah C-Closure. Jika berubah, berarti telah ditangkap.
    if type(iscclosure) == "function" then
        pcall(function()
            if loadstring and not iscclosure(loadstring) then
                detected = true
            end
        end)
    end

    -- METODE C: Pemindaian CoreGui (Mendeteksi nama GUI Logger/Spy)
    -- Mendeteksi kata kunci dari contoh script Anda ("GuiDetection_v5_6") atau alat lain seperti SimpleSpy
    local blacklistedGuiNames = {"spy", "detect", "logger", "sniffer", "dex", "monitor", "hook"}
    pcall(function()
        for _, gui in ipairs(CoreGui:GetChildren()) do
            if gui:IsA("ScreenGui") then
                local guiNameStr = string.lower(gui.Name)
                for _, badName in ipairs(blacklistedGuiNames) do
                    if string.find(guiNameStr, badName) then
                        detected = true
                        break
                    end
                end
...
-- 2. Jika Logger / Hook Terdeteksi
if isSpyDetected() then
    
    -- HAPUS SEMUA GUI DARI COREGUI (Hapus tampilan alat cheat)
    pcall(function()
        for _, gui in ipairs(CoreGui:GetChildren()) do
            if gui:IsA("ScreenGui") then
                gui:Destroy()
    ...
    -- HAPUS SEMUA GUI DARI PLAYERGUI (Membuat layar pemain menjadi kosong/rusak sementara)
    pcall(function()
    ...
    -- 3. Pemilihan Acak: Rejoin (1) atau Serverhop (2)
    local randomAction = math.random(1, 2)
    
    if randomAction == 1 then
        -- REJOIN: Memasukkan kembali pengguna ke server yang sama
        TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, LocalPlayer)
    else
        -- SERVERHOP: Melempar pengguna ke server yang berbeda secara acak
        TeleportService:Teleport(game.PlaceId, LocalPlayer)
    end
end

-- KMP DEV — KEY SYSTEM / INTRO CHECK EDITION
-- Full key-system source extracted from the stable prepend gate.
-- Active license: shows SECURITY CHECK intro before release.
-- No active license: shows normal key-validation intro.
-- Startup integrity check runs only during the intro phase.
-- If the check fails, the target remains blocked/killed.

--[[
KMP DEV — PREMIUM KEY SYSTEM V3
PREPEND / SCRIPT GATE EDITION

INSTALL:  
Add Script -> Prepend (before target)  

BEHAVIOR:  
- Target script ditahan sampai license aktif.  
- Session aktif -> target langsung dilepas.  
- Key valid -> target dilepas.  
- Key invalid -> target tetap ditahan.  
- API error -> target tetap ditahan.  
- Close / Escape -> script langsung dimatikan (terminated).  
- Bypass UI terdeteksi -> script langsung dimatikan (terminated).

UI:  
- Intro hitam pekat.  
- Intro title: Key validation.  
- Background dibuat sangat gelap.  
- Sedikit zoom/cover agar batas GUI belakang tidak terlihat.  
- Mobile responsive.
]]
...
local BASE = "https://script-protect.lovable.app"
local SYSTEM = "bd4c70f4-db8f-4d1a-8dd3-e2c7b53243a8"

local THEME = {
    Red = Color3.fromRGB(255, 30, 60),
    Red2 = Color3.fromRGB(255, 65, 85),
    Dark = Color3.fromRGB(0, 0, 0),  
    Dark2 = Color3.fromRGB(8, 10, 14),  
    White = Color3.fromRGB(245, 247, 250),  
    Gray = Color3.fromRGB(150, 155, 165),  
    Dim = Color3.fromRGB(85, 90, 100),  
    Green = Color3.fromRGB(50, 235, 125),  
    Yellow = Color3.fromRGB(255, 190, 50),
}

--============================================================
-- PREPEND EXECUTION GATE & ANTI-TAMPER
--============================================================

local GateEvent = Instance.new("BindableEvent")
local GateReleased = false
local intentionalClose = false

local function killScript(reason)
    -- 1. Hancurkan GateEvent agar fungsi holdTarget() error dan terputus
    pcall(function()
        if GateEvent then GateEvent:Destroy() end
    end)
    
    -- 2. Hasilkan pesan error fatal untuk mematikan thread script
    error("\n[KMP DEV SECURITY] " .. tostring(reason) .. "\nScript execution halted.", 0)
    
    -- 3. Loop tanpa batas untuk membekukan sisa thread
    while true do task.wait(9e9) end
end

local function releaseTarget()
    if GateReleased then  
        return  
    end  
    GateReleased = true  
    pcall(function()  
...
local function holdTarget()
    ...
    -- IMPORTANT: Execution stays here until license is accepted.  
    GateEvent.Event:Wait()
end
...
pcall(function()
    local old = CoreGui:FindFirstChild("KMPDev_KeySystemV3")  
...
pcall(function()
    if gethui then  
        local hui = gethui()  
        local old = hui:FindFirstChild("KMPDev_KeySystemV3")  
        if old then old:Destroy() end  
    end
end)
...
local function post(path, payload)
    if not httpRequest then
        return nil, "HTTP request unavailable", nil
    end
    ...
    if type(rawBody) ~= "string" or rawBody == "" then
        return nil, "empty response", status
    end

    local body
    local decodeOk = pcall(function()
        body = HttpService:JSONDecode(rawBody)
    end)

    if not decodeOk or type(body) ~= "table" then
        return nil, "invalid JSON response", status
    end
...
function KeySystem.check()
    local body, err, status = post("/api/key/check", {
        system = SYSTEM,
        user_id = tostring(LocalPlayer.UserId)
    })

    if body == nil then
        return nil, err or ("license service unavailable" .. (status and (" (HTTP " .. tostring(status) .. ")") or ""))
    end

    if body.licensed == true then return true end
    if body.licensed == false then return false end

    return nil, "API response did not contain a boolean 'licensed' value"
end

function KeySystem.getLink()
    local body, err = post("/api/key/start", {  
        system = SYSTEM,  
        user_id = tostring(LocalPlayer.UserId),  
        hwid = getHwid()  
    })  

    if body and body.success and body.link then  
        return body.link  
    end  

    return nil, body and body.error or err or "unavailable"
end

function KeySystem.redeem(key)
    local body, err = post("/api/key/redeem", {  
        system = SYSTEM,  
        user_id = tostring(LocalPlayer.UserId),  
        key = key,  
        hwid = getHwid()  
    ...
    if body and body.success then return true end  
    return false, body and body.error or err or "invalid_key"
end
...
local function getIcon(name)
    ...
    local ok, icon = pcall(function() return IconsV2.GetIcon(name) end)  
...
ScreenGui.Name = "KMPDev_KeySystemV3"
...
Blocker.Name = "InteractionBlocker"
Blocker.Size = UDim2.fromScale(1, 1)
Blocker.Position = UDim2.fromScale(0, 0)
Blocker.BackgroundColor3 = THEME.Dark
...
DimLayer.Name = "DimLayer"
DimLayer.Size = UDim2.fromScale(1, 1)
DimLayer.Position = UDim2.fromScale(0, 0)
DimLayer.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
DimLayer.BackgroundTransparency = 0.22
DimLayer.BorderSizePixel = 0
DimLayer.ZIndex = 2
DimLayer.Parent = ScreenGui

--============================================================
-- ZOOM / COVER
--============================================================

local ZoomCover = Instance.new("Frame")
ZoomCover.Name = "ZoomCover"
ZoomCover.Size = UDim2.fromScale(1.045, 1.045)
ZoomCover.Position = UDim2.fromScale(-0.0225, -0.0225)
ZoomCover.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
ZoomCover.BackgroundTransparency = 0.72
ZoomCover.BorderSizePixel = 0
ZoomCover.ZIndex = 3
ZoomCover.Parent = ScreenGui
...
Profile.Size = UDim2.fromOffset(235, 72)
Profile.Position = UDim2.fromOffset(18, 16)
...
Avatar.BackgroundColor3 = THEME.Dark2
...
AvatarStroke.Color = THEME.Red
AvatarStroke.Thickness = 2
AvatarStroke.Transparency = 0.05
AvatarStroke.Parent = Avatar

local Online = Instance.new("Frame")
Online.Size = UDim2.fromOffset(12, 12)
Online.Position = UDim2.new(1, -12, 1, -12)
Online.BackgroundColor3 = THEME.Green
Online.BorderSizePixel = 0
Online.ZIndex = 25
Online.Parent = Avatar
Instance.new("UICorner", Online).CornerRadius = UDim.new(1, 0)

local ProfileName = Instance.new("TextLabel")
ProfileName.Size = UDim2.new(1, -70, 0, 25)
ProfileName.Position = UDim2.fromOffset(70, 4)
ProfileName.BackgroundTransparency = 1
ProfileName.Text = LocalPlayer.DisplayName
ProfileName.Font = Enum.Font.GothamBold
ProfileName.TextSize = 17
ProfileName.TextColor3 = THEME.White
...
ProfileUser.Size = UDim2.new(1, -70, 0, 18)
ProfileUser.Position = UDim2.fromOffset(70, 29)
ProfileUser.BackgroundTransparency = 1
ProfileUser.Text = "@" .. LocalPlayer.Name .. "  •  " .. tostring(LocalPlayer.UserId)
ProfileUser.Font = Enum.Font.Gotham
ProfileUser.TextSize = 9
ProfileUser.TextColor3 = THEME.Gray
ProfileUser.TextXAlignment = Enum.TextXAlignment.Left
ProfileUser.TextTruncate = Enum.TextTruncate.AtEnd
ProfileUser.ZIndex = 22
ProfileUser.Parent = Profile

task.spawn(function()
    pcall(function()  
        local image = Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size150x150)  
        Avatar.Image = image  
    end)
end)
...
Status.Position = UDim2.new(1, -72, 0, 24)
Status.Size = UDim2.fromOffset(150, 38)
Status.BackgroundColor3 = THEME.Dark2
...
StatusDot.Position = UDim2.fromOffset(13, 15)
StatusDot.BackgroundColor3 = THEME.Yellow
...
Close.Position = UDim2.new(1, -18, 0, 76)
Close.Size = UDim2.fromOffset(42, 42)
Close.BackgroundColor3 = THEME.Dark2
Close.BackgroundTransparency = 0.02
...
Center.Position = UDim2.fromScale(0.5, 0.50)
Center.Size = UDim2.fromScale(0.86, 0.58)
...
CenterConstraint.MinSize = Vector2.new(300, 300)
CenterConstraint.MaxSize = Vector2.new(720, 520)
...
MainIcon.AnchorPoint = Vector2.new(0.5, 0)
MainIcon.Position = UDim2.fromScale(0.5, 0.02)
MainIcon.Size = UDim2.fromOffset(54, 54)
MainIcon.BackgroundTransparency = 1
MainIcon.ImageColor3 = THEME.Red
MainIcon.ZIndex = 15
MainIcon.Parent = Center

local ShieldIcon = getIcon("shield-check")
if ShieldIcon then MainIcon.Image = ShieldIcon end
...
IconFallback.TextSize = 36
IconFallback.TextColor3 = THEME.Red
IconFallback.Visible = ShieldIcon == nil
IconFallback.ZIndex = 16
IconFallback.Parent = MainIcon
...
Title.Position = UDim2.fromScale(0.5, 0.20)
Title.Size = UDim2.fromScale(0.95, 0.10)
...
TitleLimit.MinTextSize = 18
TitleLimit.MaxTextSize = 32
TitleLimit.Parent = Title

local Subtitle = Instance.new("TextLabel")
Subtitle.AnchorPoint = Vector2.new(0.5, 0)
Subtitle.Position = UDim2.fromScale(0.5, 0.32)
Subtitle.Size = UDim2.fromScale(0.90, 0.06)
...
SubtitleLimit.MinTextSize = 9
SubtitleLimit.MaxTextSize = 14
...
InputArea.AnchorPoint = Vector2.new(0.5, 0)
InputArea.Position = UDim2.fromScale(0.5, 0.43)
InputArea.Size = UDim2.fromScale(0.90, 0.14)
InputArea.BackgroundColor3 = THEME.Dark2
InputArea.BackgroundTransparency = 0.02
InputArea.BorderSizePixel = 0
InputArea.ZIndex = 16
InputArea.Parent = Center
Instance.new("UICorner", InputArea).CornerRadius = UDim.new(0, 14)

local InputStroke = Instance.new("UIStroke")
InputStroke.Color = THEME.Red
...
KeyIcon.ImageColor3 = THEME.Red
KeyIcon.ZIndex = 20
KeyIcon.Parent = InputArea
local KeyIconAsset = getIcon("key")
if KeyIconAsset then KeyIcon.Image = KeyIconAsset end

local KeyFallback = Instance.new("TextLabel")
KeyFallback.Size = UDim2.fromScale(1, 1)
KeyFallback.BackgroundTransparency = 1
KeyFallback.Text = "◆"
KeyFallback.Font = Enum.Font.GothamBold
KeyFallback.TextSize = 16
KeyFallback.TextColor3 = THEME.Red
KeyFallback.Visible = KeyIconAsset == nil
KeyFallback.Parent = KeyIcon

local KeyBox = Instance.new("TextBox")
KeyBox.Name = "KeyBox"
KeyBox.Size = UDim2.new(1, -64, 1, 0)
KeyBox.Position = UDim2.fromOffset(56, 0)
KeyBox.BackgroundTransparency = 1
KeyBox.BorderSizePixel = 0
KeyBox.Text = ""
KeyBox.PlaceholderText = "Enter your license key..."
KeyBox.PlaceholderColor3 = THEME.Dim
...
Buttons.Position = UDim2.fromScale(0.5, 0.61)
Buttons.Size = UDim2.fromScale(0.90, 0.15)
...
ButtonLayout.Padding = UDim.new(0.025, 0)
...
local function createButton(name, text, iconName, red)
    local button = Instance.new("TextButton")  
    button.Name = name  
    button.Size = UDim2.new(0.487, 0, 1, 0)  
    button.BackgroundColor3 = red and THEME.Red or THEME.Dark2  
    button.BackgroundTransparency = red and 0 or 0.03  
    ...
    stroke.Color = red and THEME.Red2 or Color3.fromRGB(45, 48, 57)  
    stroke.Transparency = red and 0.15 or 0.30  
    stroke.Thickness = 1  
    ...
    if asset then icon.Image = asset end  

    local fallback = Instance.new("TextLabel")  
    fallback.Size = UDim2.fromScale(1, 1)  
    fallback.BackgroundTransparency = 1  
    fallback.Text = iconName == "external-link" and "↗" or "✓"  
    fallback.Font = Enum.Font.GothamBold  
    fallback.TextSize = 20  
    fallback.TextColor3 = THEME.White  
    fallback.Visible = asset == nil  
    fallback.ZIndex = 23  
...
local GetButton, GetLabel = createButton("GetKey", "GET KEY", "external-link", false)
local VerifyButton, VerifyLabel = createButton("VerifyKey", "VERIFY KEY", "shield-check", true)
...
Message.Position = UDim2.fromScale(0.5, 0.79)
Message.Size = UDim2.fromScale(0.90, 0.07)
Message.BackgroundTransparency = 1
Message.Text = "Your license key is required before the main script can start."
Message.Font = Enum.Font.Gotham
Message.TextScaled = true
Message.TextColor3 = THEME.Gray
Message.TextXAlignment = Enum.TextXAlignment.Center
Message.ZIndex = 16
Message.Parent = Center

local MessageLimit = Instance.new("UITextSizeConstraint")
MessageLimit.MinTextSize = 9
MessageLimit.MaxTextSize = 13
...
LogContainer.AnchorPoint = Vector2.new(0, 1)
LogContainer.Position = UDim2.new(0, 26, 1, -22)
LogContainer.Size = UDim2.fromOffset(380, 145)
LogContainer.BackgroundTransparency = 1
LogContainer.BorderSizePixel = 0
LogContainer.ZIndex = 20
...
local function addLog(text, color)
    ...
    log.TextSize = 9  
    log.TextColor3 = color or THEME.Gray  
    ...
    if #logs > 8 then  
        local first = table.remove(logs, 1)  
        if first then first:Destroy() end  
    end  

    TweenService:Create(log, TweenInfo.new(0.25), {TextTransparency = 0}):Play()
end
...
ProgressBG.Position = UDim2.fromScale(0.5, 0.87)
ProgressBG.Size = UDim2.fromScale(0.58, 0.012)
ProgressBG.BackgroundColor3 = Color3.fromRGB(40, 42, 48)
...
Progress.BackgroundColor3 = THEME.Red
...
IntroIcon.Position = UDim2.fromScale(0.5, 0.39)
IntroIcon.Size = UDim2.fromOffset(72, 72)
IntroIcon.BackgroundTransparency = 1
IntroIcon.ImageColor3 = THEME.Red
IntroIcon.ImageTransparency = 1
IntroIcon.ZIndex = 105
IntroIcon.Parent = Intro
if ShieldIcon then IntroIcon.Image = ShieldIcon end

local IntroFallback = Instance.new("TextLabel")
IntroFallback.Size = UDim2.fromScale(1, 1)
IntroFallback.BackgroundTransparency = 1
IntroFallback.Text = "✓"
IntroFallback.Font = Enum.Font.GothamBold
IntroFallback.TextSize = 48
IntroFallback.TextColor3 = THEME.Red
IntroFallback.TextTransparency = 1
IntroFallback.Visible = ShieldIcon == nil
IntroFallback.ZIndex = 106
IntroFallback.Parent = IntroIcon

local IntroTitle = Instance.new("TextLabel")
IntroTitle.AnchorPoint = Vector2.new(0.5, 0.5)
IntroTitle.Position = UDim2.fromScale(0.5, 0.50)
IntroTitle.Size = UDim2.fromScale(0.8, 0.08)
IntroTitle.BackgroundTransparency = 1
IntroTitle.Text = "Key validation"
...
local IntroTitleLimit = Instance.new("UITextSizeConstraint")
IntroTitleLimit.MinTextSize = 18
IntroTitleLimit.MaxTextSize = 32
IntroTitleLimit.Parent = IntroTitle

local IntroSub = Instance.new("TextLabel")
IntroSub.AnchorPoint = Vector2.new(0.5, 0.5)
IntroSub.Position = UDim2.fromScale(0.5, 0.58)
IntroSub.Size = UDim2.fromScale(0.8, 0.05)
IntroSub.BackgroundTransparency = 1
IntroSub.Text = "SECURE LICENSE SYSTEM"
IntroSub.Font = Enum.Font.Gotham
IntroSub.TextScaled = true
IntroSub.TextColor3 = THEME.Gray
IntroSub.TextTransparency = 1
IntroSub.ZIndex = 105
IntroSub.Parent = Intro

local IntroSubLimit = Instance.new("UITextSizeConstraint")
IntroSubLimit.MinTextSize = 8
IntroSubLimit.MaxTextSize = 13
IntroSubLimit.Parent = IntroSub

--============================================================
-- INTRO MODE / SESSION CHECK
--============================================================

local IntroMode = "LICENSE"
local IntroCheckPassed = false
local IntroCheckFailed = false

local IntroNotice = Instance.new("TextLabel")
IntroNotice.AnchorPoint = Vector2.new(0.5, 0.5)
IntroNotice.Position = UDim2.fromScale(0.5, 0.625)
IntroNotice.Size = UDim2.fromScale(0.82, 0.055)
IntroNotice.BackgroundTransparency = 1
IntroNotice.Text = "For safety, do not execute other scripts during this verification."
IntroNotice.Font = Enum.Font.Gotham
IntroNotice.TextScaled = true
IntroNotice.TextColor3 = THEME.Dim
IntroNotice.TextTransparency = 1
IntroNotice.TextXAlignment = Enum.TextXAlignment.Center
IntroNotice.ZIndex = 105
IntroNotice.Parent = Intro

local IntroNoticeLimit = Instance.new("UITextSizeConstraint")
IntroNoticeLimit.MinTextSize = 8
IntroNoticeLimit.MaxTextSize = 12
IntroNoticeLimit.Parent = IntroNotice

local IntroProgressBG = Instance.new("Frame")
IntroProgressBG.AnchorPoint = Vector2.new(0.5, 0.5)
IntroProgressBG.Position = UDim2.fromScale(0.5, 0.705)
IntroProgressBG.Size = UDim2.fromScale(0.34, 0.012)
IntroProgressBG.BackgroundColor3 = Color3.fromRGB(40, 42, 48)
IntroProgressBG.BackgroundTransparency = 0.15
IntroProgressBG.BorderSizePixel = 0
IntroProgressBG.ZIndex = 105
IntroProgressBG.Parent = Intro
Instance.new("UICorner", IntroProgressBG).CornerRadius = UDim.new(1, 0)

local IntroProgress = Instance.new("Frame")
IntroProgress.Size = UDim2.fromScale(0, 1)
IntroProgress.BackgroundColor3 = THEME.Red
IntroProgress.BorderSizePixel = 0
IntroProgress.ZIndex = 106
IntroProgress.Parent = IntroProgressBG
Instance.new("UICorner", IntroProgress).CornerRadius = UDim.new(1, 0)

--============================================================
-- INTRO ANIMATION
--============================================================

local function setIntroMode(mode)
    IntroMode = mode
    if mode == "CHECK" then
        IntroTitle.Text = "SECURITY CHECK"
        IntroSub.Text = "ACTIVE LICENSE DETECTED"
        IntroNotice.Text = "Verifying session integrity. Do not execute other scripts during this check."
        IntroNotice.TextColor3 = THEME.Gray
    else
        IntroTitle.Text = "Key validation"
        IntroSub.Text = "SECURE LICENSE SYSTEM"
        IntroNotice.Text = "For safety, do not execute other scripts during this verification."
...
--============================================================
-- SAFE SESSION / UI INTEGRITY CHECK
--============================================================

local function runSessionIntegrityCheck()
    local checks = {
        {
            "ScreenGui",
            function()
        ...
        },
        {
            "Intro",
            function()
                return Intro ~= nil and Intro.Parent == ScreenGui
            end
        },
        {
            "KeySystem",
            function()
                return type(KeySystem) == "table"
                    and type(KeySystem.check) == "function"
                    and type(KeySystem.redeem) == "function"
                    and type(KeySystem.getLink) == "function"
            end
        },
        {
            "Execution gate",
            function()
                return GateReleased == false
            end
        }
    }
    ...
    for _, item in ipairs(checks) do
        ...
        if not ok or result ~= true then
            return false, "Integrity check failed: " .. item[1]
        end
    end
    return true
end

local startupLicenseDone = false
local startupLicenseResult = false

local function playIntro()
    IntroCheckPassed = false
    IntroCheckFailed = false

    addLog(
        IntroMode == "CHECK" and "Active session detected." or "Initializing secure session...",
        IntroMode == "CHECK" and THEME.Green or THEME.Gray
    )

    TweenService:Create(IntroIcon, TweenInfo.new(0.65, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
    TweenService:Create(IntroFallback, TweenInfo.new(0.65), {TextTransparency = 0}):Play()
    task.wait(0.18)
    TweenService:Create(IntroTitle, TweenInfo.new(0.55, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
    task.wait(0.12)
    TweenService:Create(IntroSub, TweenInfo.new(0.45), {TextTransparency = 0}):Play()
    task.wait(0.12)
    TweenService:Create(IntroNotice, TweenInfo.new(0.45), {TextTransparency = 0}):Play()
    task.wait(0.18)

    local progressTween = TweenService:Create(
        IntroProgress,
        TweenInfo.new(3.75, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
        {Size = UDim2.fromScale(1, 1)}
    )
    progressTween:Play()

    if IntroMode == "CHECK" then
        while not startupLicenseDone do task.wait(0.10) end

        if startupLicenseResult == nil then
            addLog("License service did not respond.", THEME.Yellow)
            IntroTitle.Text = "LICENSE SERVICE UNAVAILABLE"
            IntroSub.Text = "CHECK COULD NOT BE COMPLETED"
            IntroNotice.Text = "Please retry when the license service is reachable."
            IntroNotice.TextColor3 = THEME.Yellow
            IntroProgress.BackgroundColor3 = THEME.Yellow
            return false
        elseif startupLicenseResult == false then
            addLog("No active license detected.", THEME.Yellow)
            IntroTitle.Text = "LICENSE VERIFICATION"
            IntroSub.Text = "NO ACTIVE LICENSE"
            IntroNotice.Text = "Please enter your license key to continue."
            IntroNotice.TextColor3 = THEME.Yellow
        else
            addLog("Checking execution environment...", THEME.Gray)
            task.wait(0.30)
            addLog("Validating active session...", THEME.Gray)
            task.wait(0.30)
            addLog("Running startup integrity checks...", THEME.Gray)

            local passed, reason = runSessionIntegrityCheck()

            if not passed then
                IntroCheckFailed = true
                addLog(reason, THEME.Red)
                IntroSub.Text = "SECURITY CHECK FAILED"
                IntroNotice.Text = "The script was not started because the startup state changed."
                IntroNotice.TextColor3 = THEME.Red
                IntroProgress.BackgroundColor3 = THEME.Red
                task.wait(0.65)
                
                -- [KILL SCRIPT] Mematikan thread jika check gagal
                killScript("Security check gagal: " .. tostring(reason))
                return false
            end

            IntroCheckPassed = true
            addLog("Session integrity verified.", THEME.Green)
            IntroTitle.Text = "VERIFIED"
            IntroSub.Text = "ACTIVE LICENSE CONFIRMED"
            IntroNotice.Text = "Verification complete. Startup protection is now ending."
            IntroNotice.TextColor3 = THEME.Green
        end
    else
        task.wait(0.35)
        addLog("Checking execution environment...", THEME.Gray)
        task.wait(0.35)
        addLog("Connecting license service...", THEME.Gray)
        task.wait(0.40)
        addLog("Checking active license...", THEME.Gray)
    end

    task.wait(0.85)

    TweenService:Create(Intro, TweenInfo.new(0.65, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1}):Play()
    TweenService:Create(IntroIcon, TweenInfo.new(0.35), {ImageTransparency = 1}):Play()
    TweenService:Create(IntroFallback, TweenInfo.new(0.35), {TextTransparency = 1}):Play()
    TweenService:Create(IntroTitle, TweenInfo.new(0.35), {TextTransparency = 1}):Play()
    TweenService:Create(IntroSub, TweenInfo.new(0.35), {TextTransparency = 1}):Play()
    TweenService:Create(IntroNotice, TweenInfo.new(0.35), {TextTransparency = 1}):Play()
...
local function updateResponsive()
    ...
    if viewport.X <= 520 then  
        Profile.Size = UDim2.fromOffset(205, 65)  
        Profile.Position = UDim2.fromOffset(12, 12)  
        Avatar.Size = UDim2.fromOffset(52, 52)  
        ProfileName.Position = UDim2.fromOffset(62, 2)  
        ProfileName.TextSize = 14  
        ProfileUser.Position = UDim2.fromOffset(62, 27)  
        ProfileUser.TextSize = 8  
        Status.Position = UDim2.new(1, -60, 0, 15)  
        Status.Size = UDim2.fromOffset(132, 34)  
        StatusText.TextSize = 8  
        Close.Position = UDim2.new(1, -12, 0, 58)  
        Close.Size = UDim2.fromOffset(38, 38)  
        Center.Position = UDim2.fromScale(0.5, 0.50)  
        Center.Size = UDim2.fromScale(0.92, 0.56)  
        CenterConstraint.MinSize = Vector2.new(290, 300)  
        CenterConstraint.MaxSize = Vector2.new(480, 500)  
        Buttons.Size = UDim2.fromScale(0.90, 0.15)  
        LogContainer.Position = UDim2.new(0, 15, 1, -15)  
        LogContainer.Size = UDim2.fromOffset(math.min(viewport.X * 0.72, 340), 120)  
    else  
        Profile.Size = UDim2.fromOffset(235, 72)  
        Profile.Position = UDim2.fromOffset(18, 16)  
        Avatar.Size = UDim2.fromOffset(58, 58)  
...
local function setMessage(text, color)
    Message.Text = text  
    Message.TextColor3 = color or THEME.Gray
end
...
local function buttonLoading(button, label, loading)
    if loading then  
        label.Text = "PROCESSING..."  
        button.Active = false  
    else  
        if button == GetButton then  
            label.Text = "GET KEY"  
        else  
...
GetButton.Activated:Connect(function()
    if busyGet then return end  
    busyGet = true  
    buttonLoading(GetButton, GetLabel, true)  
    setStatus("GETTING KEY", THEME.Yellow)  
    setMessage("Generating your secure key link...", THEME.Gray)  
    addLog("Requesting key link...", THEME.Gray)  
    Progress.Size = UDim2.fromScale(0, 1)  

    TweenService:Create(Progress, TweenInfo.new(0.8, Enum.EasingStyle.Linear), {Size = UDim2.fromScale(0.72, 1)}):Play()  

    local link, err = KeySystem.getLink()  

    if link then  
        local copied = copyToClipboard(link)  
        if copied then  
            setStatus("LINK COPIED", THEME.Green)  
            setMessage("Key link copied to clipboard. Open it in your browser.", THEME.Green)  
            addLog("Key link copied successfully.", THEME.Green)  
        else  
            setStatus("LINK READY", THEME.Green)  
            setMessage(tostring(link), THEME.White)  
            addLog("Clipboard unavailable — link displayed.", THEME.Yellow)  
        end  
    else  
        setStatus("REQUEST ERROR", THEME.Red)  
        setMessage("Unable to generate key: " .. tostring(err), THEME.Red)  
        addLog("Key request failed: " .. tostring(err), THEME.Red)  
    end  

    TweenService:Create(Progress, TweenInfo.new(0.25), {Size = UDim2.fromScale(0, 1)}):Play()  
    buttonLoading(GetButton, GetLabel, false)  
    busyGet = false  
end)
...
local function unlock()
    setStatus("LICENSE ACTIVE", THEME.Green)  
    setMessage("Verification successful. Loading...", THEME.Green)  
    addLog("License accepted. Releasing target script.", THEME.Green)  
    task.wait(0.35)  
    releaseTarget()  
...
local busyVerify = false

VerifyButton.Activated:Connect(function()
    if busyVerify then return end  

    local key = tostring(KeyBox.Text or ""):gsub("%s", "")  
    if key == "" then  
        setStatus("KEY REQUIRED", THEME.Yellow)  
        setMessage("Please enter your license key first.", THEME.Yellow)  
        addLog("Verification cancelled: empty key.", THEME.Yellow)  
        return  
    end  

    busyVerify = true  
    buttonLoading(VerifyButton, VerifyLabel, true)  
    GetButton.Active = false  
    KeyBox.TextEditable = false  
    setStatus("VERIFYING", THEME.Yellow)  
    setMessage("Verifying your license...", THEME.Gray)  
    addLog("Sending verification request...", THEME.Gray)  
    Progress.Size = UDim2.fromScale(0, 1)  

    local progressTween = TweenService:Create(Progress, TweenInfo.new(1.0, Enum.EasingStyle.Linear), {Size = UDim2.fromScale(0.82, 1)})  
    progressTween:Play()  

    local ok, err = KeySystem.redeem(key)  

    if ok then  
        progressTween:Cancel()  
        TweenService:Create(Progress, TweenInfo.new(0.25, Enum.EasingStyle.Linear), {Size = UDim2.fromScale(1, 1)}):Play()  
        task.wait(0.25)  
        unlock()  
        return  
    end  
    ...
    progressTween:Cancel()  
    TweenService:Create(Progress, TweenInfo.new(0.25), {Size = UDim2.fromScale(0, 1)}):Play()  
    setStatus("INVALID KEY", THEME.Red)  
    setMessage("Key rejected: " .. tostring(err), THEME.Red)  
    addLog("License rejected: " .. tostring(err), THEME.Red)  

    buttonLoading(VerifyButton, VerifyLabel, false)  
...
Close.Activated:Connect(function()
    if GateReleased then return end  

    setStatus("BLOCKED", THEME.Red)  
    setMessage("Key validation is required before the script can continue.", THEME.Red)  
    addLog("Interface closed. Target remains blocked.", THEME.Red)  
    task.wait(0.25)  

    intentionalClose = true  -- Tandai bahwa ini ditutup paksa secara disengaja
    
    if ScreenGui and ScreenGui.Parent then  
        ScreenGui:Destroy()  
    end  

    -- [KILL SCRIPT] Matikan thread jika user menekan Close
    killScript("Interface ditutup oleh user. Script dihentikan.")
end)
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == Enum.KeyCode.Escape then  
        if ScreenGui and ScreenGui.Parent and not GateReleased then  
            
            intentionalClose = true -- Tandai penutupan
            ScreenGui:Destroy()  
            
            -- [KILL SCRIPT] Matikan thread jika user menekan ESC
            killScript("Interface ditutup via ESC. Script dihentikan.")
        end  
    end  
end)

--============================================================
-- ANTI-TAMPER / GUI MONITOR
--============================================================
task.spawn(function()
    -- Deteksi bypass: Jika GUI utama dihapus paksa dari luar sebelum key valid
    ScreenGui.AncestryChanged:Connect(function(_, newParent)
        if not GateReleased and not intentionalClose and newParent == nil then
            killScript("UI dihapus secara paksa (Bypass)! Script dimatikan.")
        end
    end)

    -- Backup check berjaga-jaga
    while not GateReleased do
        task.wait(0.5)
        if not intentionalClose and (not ScreenGui or ScreenGui.Parent == nil) then
            killScript("Manipulasi interface terdeteksi! Script dimatikan.")
        end
    end
end)
...
addLog("Secure interface initialized.", THEME.Gray)

--============================================================
-- START
--============================================================

local STARTUP_CHECK_ATTEMPTS = 15
local STARTUP_CHECK_DELAY = 1.0

local function checkLicenseWithRetry()
    local lastError = "unknown"

    for attempt = 1, STARTUP_CHECK_ATTEMPTS do
        local ok, licensed, err = pcall(function()
            local result, reason = KeySystem.check()
            return result, reason
        end)

        if ok and licensed == true then return true end
        if ok and licensed == false then return false end

        lastError = tostring(err or licensed or "no definitive response")
        addLog("Waiting for license service... (" .. tostring(attempt) .. "/" .. tostring(STARTUP_CHECK_ATTEMPTS) .. ")", THEME.Gray)

        if attempt < STARTUP_CHECK_ATTEMPTS then
            task.wait(STARTUP_CHECK_DELAY)
        end
    end

    addLog("License check timeout: " .. lastError, THEME.Yellow)
    return nil
end

task.spawn(function()
    Main.Visible = false
    startupLicenseDone = false
    startupLicenseResult = false

    task.spawn(function()
        startupLicenseResult = checkLicenseWithRetry()
        startupLicenseDone = true
    end)

    setIntroMode("CHECK")
    local introOk = playIntro()

    if startupLicenseResult == true then
        if introOk and IntroCheckPassed and not IntroCheckFailed then
            releaseTarget()
        end
        if ScreenGui and ScreenGui.Parent then
            ScreenGui:Destroy()
    ...
    if startupLicenseResult == false then
        if ScreenGui and ScreenGui.Parent and not GateReleased then
            Main.Visible = true
        end
        return
...
--============================================================
-- TARGET SCRIPT CONTINUES HERE
--============================================================
-- (PASTE SCRIPT UTAMA ANDA DI BAWAH GARIS INI)
...
if ModernV2 then
    ...
    Window:CreateHomeTab({
        ...
        Changelog = {
            ...
            {
                Title = "KysHub VD v1.5.3",
                Description = "Added Auto Drop All Pallets, Break All Pallet, Show Hook Counter. Removed Copy Avatar.",
            },
            {
                Title = "KysHub VD v1.5.2",
                Description = "Added internal info panels and Moonwalk external toggle button with lock support.",
            },
            {
                Title = "KysHub VD v1.5.1",
                Description = "Added Silent Aim Flashlight with laser support and Premium-only access.",
            },
            {
                Title = "KysHub VD v1.5.0",
                Description = "Added ToF features, Hide Survivor Icon, and Show Ping & FPS.",
            },
            {
                Title = "KysHub VD v1.4.9",
                Description = "Added Killer Perks Display and Predict Map display, refined Predict Map UI output.",
            },
            {
                Title = "KysHub VD v1.4.8",
                Description = "Added Spectator Counter, updated Player Emote logic, fixed Spear tabbox icon.",
            },
            {
                Title = "KysHub VD v1.4.7",
                Description = "Added Apply Korless cosmetic morph, fixed Instant Skillcheck & Cooldown Bypass logic.",
            },
            {
                Title = "KysHub VD v1.4.6",
                Description = "Added Kyst Killer Display, Fixed TP to Gen scoping issues, Optimized metatable hooks.",
            },
            {
                Title = "KysHub VD v1.4.5",
                Description = "Added No Cutscene and Premium lock updates.",
            },
            {
                Title = "KysHub VD v1.4.4",
                Description = "Added Auto Kick Generator, Fixed Cooldown & Freezing issues.",
            },
            {
                Title = "KysHub VD v1.4.3",
                Description = "Improved Auto Parry, Added Auto Parry Agresif, Fixed Auto Crouch.",
            },
        },
        UIChangelog = {
            {
...
print("KYS HUB Violence District v1.5.6 Loaded")
pcall(function()
    VD_Notify("KysHub", "Violence District v1.5.6 Loaded Successfully!", 5)
end)
